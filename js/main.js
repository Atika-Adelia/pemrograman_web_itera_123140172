...
const resetFormDanMode = () => {
    ...
    inputEditId.value = ''; // Kosongkan ID edit
    btnSubmit.textContent = 'Tambah Jadwal'; // Kembalikan teks tombol
...
form.addEventListener('submit', async (event) => {
    ...
    const lokasi = inputLokasi.value; // BARU
...
listContainer.addEventListener('click', async (event) => {
    ...
    if (event.target.classList.contains('btn-delete')) {
        const idHapus = Number(event.target.dataset.id); //logika untuk menghapus jadwalnya
        try {
            let daftarJadwal = await ambilData(KEY_PENYIMPANAN);
        ...
        const idEdit = Number(event.target.dataset.id); //kalau ini untuk mengeditnya

        try {
            const daftarJadwal = await ambilData(KEY_PENYIMPANAN);
            const jadwalEdit = daftarJadwal.find(jadwal => jadwal.id === idEdit);

            if (jadwalEdit) { //jadwal diedit
